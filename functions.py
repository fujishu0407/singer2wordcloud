...
def draw_wordcloud(df,col_name_noun,col_name_quant,fig_title,masking,color,mask_file,font_file):
    word_freq_dict = {}
    ...
    if masking:
        ...
        image_colors = ImageColorGenerator(tele_mask)
    else:
        tele_mask = None
    df = pd.json_normalize(word_freq_dict)
    df.to_csv("./tmp.csv")
